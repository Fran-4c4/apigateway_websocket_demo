...
"""
Unit tests for lambda_websocket.py.
"""
...
def test_handle_connect(make_stubber, error_code, status_code):
    ...
    dynamodb_stubber.stub_put_item(
        table.name, {'connection_id': connection_id, 'participant_id': participant_id},
        error_code=error_code)

    got_status_code = lambda_websocket.handle_connect(participant_id, table, connection_id)
...
def test_handle_disconnect(make_stubber, error_code, status_code):
    ...
    got_status_code = lambda_websocket.handle_disconnect(table, connection_id)
    assert got_status_code == status_code
...
def test_handle_message(
    ...
    dynamodb_stubber.stub_get_item(
        ...
        {'connection_id': connection_id, 'participant_id': participant_id},
    ...
    if error_method != 'stub_scan':
        apig_management_stubber.stub_post_to_connection(
            f'{participant_id}: {msg}'.encode('utf-8'), other_connection_id,
            error_code=error_code if error_method == 'stub_post_to_connection'
            else None)

    got_status_code = lambda_websocket.handle_message(
        table, connection_id, {'msg': msg}, apig_management_client)
    assert got_status_code == status_code


@pytest.mark.parametrize(
    'table_name,route,connection_id,conn_token,msg_body,domain,stage,status_code', [
...
def test_lambda_handler(
        monkeypatch, table_name, route, connection_id, conn_token, msg_body, domain,
        stage, status_code):
    def verify_handle_connect(uname, tbl, conn):
        assert uname == conn_token
    ...
    monkeypatch.setattr(lambda_websocket, 'handle_connect', verify_handle_connect)
    monkeypatch.setattr(lambda_websocket, 'handle_disconnect', verify_handle_disconnect)
    monkeypatch.setattr(lambda_websocket, 'handle_message', verify_handle_message)
    ...
    event = {
        ...
        'queryStringParameters': {'conn_token': conn_token},
        'body': msg_body
    }

    got_response = lambda_websocket.lambda_handler(event, None)
