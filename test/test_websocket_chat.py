...
from app_cfg import ApiGatewayHelper
...
def test_create_api(make_stubber, error_code):
    ...
    sock_gate = ApiGatewayHelper(api_name, apigatewayv2_client)
...
def test_add_connection_permissions(
    ...
    sock_gate = ApiGatewayHelper('test-api', apigatewayv2_client)
...
def test_delete_api(make_stubber, error_code):
    ...
    if error_code is None:
        sock_gate.delete_websocket_api()
    else:
        with pytest.raises(ClientError) as exc_info:
            sock_gate.delete_websocket_api()
        assert exc_info.value.response['Error']['Code'] == error_code
